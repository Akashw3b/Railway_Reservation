...
def ticket_booking():
    ...
    mycon = mysql.connector.connect(
        host='localhost', user='root', passwd='', database='railway')
    cursor = mycon.cursor()
    ...
    v = a[Gender]
    s1 = "insert into railway values('{}',{},{},'{}','{}','{}','{}')".format(
        nm, phno, age, v, fr, to, date)
    cursor.execute(s1)
...
def checking_2():
    ...
    b = input('PASS WORD:')
    try:
        s1 = "select user_name from user_accounts where password='{}'".format(
            b)
        cursor.execute(s1)
        data = cursor.fetchall()[0]
        data = list(data)
        if data[0] == a:
            ...
            s1 = "select user_name from user_accounts where password='{}'".format(
                b)
            c1 = "select fname,lname from user_accounts where password='{}'".format(
                b)
            cursor.execute(c1)
            ...
            if data[0] == a:
                ...
                s1 = "select fname,lname,phno,gender,dob,age from user_accounts where password='{}'".format(
                    b)
                cursor.execute(s1)
                data = cursor.fetchall()[0]
                data = list(data)
                ...
                if vi == 1:
                    b1 = "delete from user_accounts where password = '{}'".format(
                        b)
                    cursor.execute(b1)
                    return True
                elif vi == 2:
